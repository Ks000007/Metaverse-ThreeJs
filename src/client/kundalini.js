...
var baseActions2 = {
    Akarna: { weight: 0 },
    ArmatureAction: { weight: 0 },
    bhujangasana: { weight: 0 },
};
...
function init() {
    ...
    var baseActions = baseActions2;
    console.log("sitting");
    loader.load('assets/kundalini.glb', function(gltf) {
...
const akarna = new THREE.PositionalAudio(listener);
audioLoader.load('sounds/akarna dhanuasana.mp3', function(buffer) {
    akarna.setBuffer(buffer);
    akarna.setRefDistance(20);

});
const bhujangasana = new THREE.PositionalAudio(listener);


audioLoader.load('sounds/bhujangasana.mp3', function(buffer) {
    bhujangasana.setBuffer(buffer);
    bhujangasana.setRefDistance(20);

});
const ustrasana = new THREE.PositionalAudio(listener);

audioLoader.load('sounds/ustrasana.mp3', function(buffer) {
    ustrasana.setBuffer(buffer);
    ustrasana.setRefDistance(20);
...
function animate() {
    ...
    //-8, y: 0.32, z: -3
    if (baseActions2.Akarna.weight === 1) {
        model.position.set(-8, 0.32, -3)
        akarna.play();
        ustrasana.pause();
        bhujangasana.pause();
    }
    if (baseActions2.ArmatureAction.weight === 1) {

        model.position.set(-8, -0.25, -3.6);
        ustrasana.play();
        akarna.pause();
        bhujangasana.pause()
    }
    if (baseActions2.bhujangasana.weight === 1) {
        model.position.set(-8, 0.32, -4.3);
        bhujangasana.play();
        akarna.pause();
        ustrasana.pause();
    }
