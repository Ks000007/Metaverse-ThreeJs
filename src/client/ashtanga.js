...
function init() {
    ...
    yogabutton.innerHTML = 'Resorative';
...
const hasta = new THREE.PositionalAudio(listener);
...
audioLoader.load('sounds/utthita hasta padangustana.mp3', function(buffer) {
    hasta.setBuffer(buffer);
    hasta.setRefDistance(20);

});
const utkatasana = new THREE.PositionalAudio(listener);


audioLoader.load('sounds/utkatasana.mp3', function(buffer) {
    utkatasana.setBuffer(buffer);
    utkatasana.setRefDistance(20);
...
function animate() {
    ...
    if (baseActions2.utkatasan.weight === 1) {
        utkatasana.play();
        hasta.pause();
    }
    if (baseActions2.utkatasan.weight === 1) {
